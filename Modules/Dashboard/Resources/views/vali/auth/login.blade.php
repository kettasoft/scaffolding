...
<head>
    ...
    <title>@lang('admin.auth.login.title') | {{ config('app.name', 'Laravel') }}</title>
    <!-- Tell the browser to be responsive to screen width -->
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <!-- CSRF Token -->
    <meta name="csrf-token" content="{{ csrf_token() }}">
    @if(Locales::getDir() == 'rtl')
        <link rel="stylesheet" href="{{ asset('css/vali.rtl.css') }}">
    @else
        <link rel="stylesheet" href="{{ asset('css/vali.css') }}">
    @endif
</head>
<body>
    ...
    <div class="login-box">
        <form class="login-form" action="{{ route('login') }}" method="post">
            ...
            <div class="form-group">
                <label class="control-label">@lang('admin.auth.login.email')</label>
            ...
            <div class="form-group">
                <label class="control-label">@lang('admin.auth.login.password')</label>
        ...
        <form class="forget-form" action="{{ route('password.email') }}" method="post">
            ...
            <h3 class="login-head">
                ...
                @lang('admin.auth.forget.title')
            </h3>
            <div class="form-group">
                <label class="control-label">@lang('admin.auth.forget.email')</label>
                <input class="form-control{{ $errors->has('email') ? ' is-invalid' : '' }}"
                       type="text"
                       name="email"
                       value="{{ old('email') }}"
                       autofocus
                       placeholder="@lang('admin.auth.forget.email')">
            </div>
            <div class="form-group btn-container">
                <button class="btn btn-primary btn-block" type="submit">
